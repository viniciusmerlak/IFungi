...
void WebServerHandler::handleFirebaseConfig() {
    ...
    "<style>"
    ":root{--success:#4CAF50;--light:#f1f8e9;--dark:#1B5E20;}"
    "*{margin:0;padding:0;box-sizing:border-box;font-family:'Segoe UI',Roboto,Oxygen,Ubuntu,sans-serif;}"
    "body{background:linear-gradient(135deg,#E8F5E9 0%,#C8E6C9 100%);min-height:100vh;display:flex;justify-content:center;align-items:center;padding:20px;text-align:center;}"
    ".success-card{background:white;border-radius:12px;box-shadow:0 8px 24px rgba(0,0,0,0.1);padding:2.5rem;max-width:450px;width:100%;animation:fadeInUp 0.5s ease-out;}"
    "@keyframes fadeInUp{from{opacity:0;transform:translateY(20px);}to{opacity:1;transform:translateY(0);}}"
    "h1{color:var(--dark);margin-bottom:1rem;font-size:2rem;font-weight:600;}"
    "p{color:#2E7D32;font-size:1.1rem;margin-bottom:1.5rem;}"
    ".success-icon{width:80px;height:80px;margin:0 auto 1.5rem;display:block;color:var(--success);}"
    ".btn-return{display:inline-block;background:var(--success);color:white;text-decoration:none;padding:0.8rem 1.5rem;border-radius:8px;font-weight:500;transition:all 0.3s;margin-top:1rem;box-shadow:0 4px 6px rgba(0,0,0,0.1);}"
    ".btn-return:hover{background:#388E3C;transform:translateY(-2px);box-shadow:0 6px 12px rgba(0,0,0,0.15);}"
    "</style>"
    "</head>"
    "<body>"
    ...
    "</svg>"
    "<h1>Sucesso!</h1>"
    "<p>Autenticado no Firebase com sucesso</p>"
    ...
    "</html>"
        ...
        "<meta charset='UTF-8'>"
        "<meta name='viewport' content='width=device-width, initial-scale=1.0'>"
        "<title>Configurar Firebase</title>"
        "<style>"
        ":root{--primary:#ff5722;--secondary:#e64a19;--light:#f5f5f5;--dark:#212121;--error:#f44336;}"
        "*{margin:0;padding:0;box-sizing:border-box;font-family:'Segoe UI',Roboto,Oxygen,Ubuntu,sans-serif;}"
        "body{background:linear-gradient(135deg,#f5f5f5 0%,#e0e0e0 100%);min-height:100vh;display:flex;justify-content:center;align-items:center;padding:20px;}"
        ".container{background:white;border-radius:12px;box-shadow:0 8px 24px rgba(0,0,0,0.1);width:100%;max-width:420px;padding:2.5rem;text-align:center;}"
        "h1{color:var(--dark);margin-bottom:1.5rem;font-size:1.8rem;font-weight:600;}"
        ".firebase-logo{width:60px;height:60px;margin-bottom:1.2rem;}"
        ".form-group{margin-bottom:1.2rem;text-align:left;}"
        "label{display:block;margin-bottom:0.5rem;color:var(--dark);font-weight:500;font-size:0.95rem;}"
        "input{width:100%;padding:0.8rem 1rem;border:2px solid #e0e0e0;border-radius:8px;font-size:1rem;transition:all 0.3s ease;}"
        "input:focus{border-color:var(--primary);outline:none;box-shadow:0 0 0 3px rgba(255,87,34,0.2);}"
        "input::placeholder{color:#9e9e9e;}"
        "button{background:linear-gradient(to right,var(--primary),var(--secondary));color:white;border:none;padding:0.9rem;width:100%;border-radius:8px;font-size:1rem;font-weight:600;cursor:pointer;transition:all 0.3s;margin-top:0.5rem;box-shadow:0 4px 6px rgba(0,0,0,0.1);}"
        "button:hover{background:linear-gradient(to right,var(--secondary),var(--primary));transform:translateY(-2px);box-shadow:0 6px 12px rgba(0,0,0,0.15);}"
        ".footer{margin-top:1.5rem;color:#757575;font-size:0.85rem;}"
        "@media (max-width:480px){.container{padding:1.8rem;}}"
        "</style>"
        "</head>"
        "<body>"
        "<div class='container'>"
        "<svg class='firebase-logo' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'>"
        ...
        "</div>"
        "<button type='submit'>Acessar Firebase</button>"
        "</form>"
